...
class ResNetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, downsample=False):
        ...
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1 if not downsample else 2, padding=1)
...
class DCGANTrainer:
    def __init__(self, data_path, ann_file_path, img_size=64, channels=1, latent_dim=100, n_epochs=10, batch_size=64, lr=0.0002, b1=0.5, b2=0.999, sample_interval=400, oversample='N', patience=5):
        ...
        def __init__(self, outer_instance):
            ...
            self.model = nn.Sequential(
                *discriminator_block(outer_instance.channels, 16, bn=False),
                ResNetBlock(16, 32, downsample=True),  # Replace a conv layer with a ResNet block
                *discriminator_block(32, 64),
                ResNetBlock(64, 128, downsample=True),  # Replace another conv layer
            ...
            self.class_layer = nn.Sequential(nn.Linear(64 * ds_size ** 2, 1), nn.Sigmoid())
        ...
        for epoch in range(self.n_epochs):
            for i, (imgs, labels) in enumerate(self.data_loader):
                ...
                print(
                    ...
                    % (epoch, self.n_epochs, i, len(self.data_loader), d_loss.item(), g_loss.item())
                )
    ...
    def save_model(self, discriminator, epoch, model_path="dcgan_resnet_models"): #generator paramter removed
        """
        ...
        #torch.save(generator.state_dict(), f"{model_path}/generator_epoch_{epoch}.pth")
        torch.save(discriminator.state_dict(), f"{model_path}/discriminator_epoch_{epoch}.pth")
        print(f"Models saved at epoch {epoch} to {model_path}")

    def load_model(discriminator, model_path="dcgan_models", epoch=None): #generator
        """
        ...
        - generator (torch.nn.Module): The generator model.
        - discriminator (torch.nn.Module): The discriminator model.
        - model_path (str): Directory from which to load the model files.
        - epoch (int, optional): Epoch number of the model to load. If None, load the latest model.
        """
        # Determine which epoch's model to load
        if epoch is None:
            # Find the latest saved model files if epoch is not specified
            #generator_files = [f for f in os.listdir(model_path) if "generator_epoch_" in f]
            discriminator_files = [f for f in os.listdir(model_path) if "discriminator_epoch_" in f]
            #latest_gen_file = max(generator_files, key=lambda x: int(x.split('_')[-1].split('.')[0]))
            latest_disc_file = max(discriminator_files, key=lambda x: int(x.split('_')[-1].split('.')[0]))
        else:
            #latest_gen_file = f"generator_epoch_{epoch}.pth"
            latest_disc_file = f"discriminator_epoch_{epoch}.pth"
        
        # Load the models
        #generator.load_state_dict(torch.load(os.path.join(model_path, latest_gen_file)))
        discriminator.load_state_dict(torch.load(os.path.join(model_path, latest_disc_file)))
        print(f"Loaded models from {latest_disc_file}") #{latest_gen_file} and 
